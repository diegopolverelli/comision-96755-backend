...
export class ProductsRouter extends CustomRouter{
    init(){
        this.get1("/", midd01, (req, res)=>{

            if(req.query.error){
                res.badrequest("error de prueba...!!!")
            }

            // res.setHeader('Content-Type','application/json');
            // return res.status(200).json({payload:productos});
            res.success("listado de productos", productos)
        })

        this.get1("/informe", midd01, (req, res)=>{

            if(req.query.error){
                res.badrequest("error de prueba...!!!")
            }

            throw new Error("error prueba2")
